...
if (role === 'Admin' || role === 'Staff') {
  // Both Admin and Staff likely arrived from the All Bookings page.
  backLink.href = 'all-bookings.html';
  backLink.textContent = '← All Bookings';
}
// Passenger: leave the default ("← My Bookings") from the HTML.
...
async function renderDetails(b) {
  ...
  // "Go Back" inside the details/cancel-summary card.
  // Admin/Staff -> All Bookings, Passenger -> My Bookings.
  var goBackHref;
  if (role === 'Passenger')      goBackHref = 'my-bookings.html';
  else                            goBackHref = 'all-bookings.html';
  document.getElementById('go-back-link').href = goBackHref;
...
async function confirmCancellation() {
  ...
  if (booking.date < today) {
    showToast('Cannot cancel a trip that has already departed.', 'error');
    return;
  }
...
async function showSuccess(refund, booking) {
  ...
  if (role === 'Admin' || role === 'Staff') {
    actionsHtml =
      '<a href="all-bookings.html" class="btn btn-primary">All Bookings</a>' +
